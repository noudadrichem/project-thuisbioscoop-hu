...
def tableHeading(root, index, text):
    ...
    label4 = Label(
        ...
        background=bg,
        font=("Open Sans", 8, "bold"))
    label4.grid(row=1, column=index+5, columnspan=2, sticky='w')

def movieLabel(movie, counter, idx, window, columnNumber):
    movieTitle = movie['title']
    startTijd = formattedUnix(movie['start'])
    eindTijd = formattedUnix(movie['eind'])
    aanbieder = movie['aanbieder']

    lefAlignedLabel(movieTitle, counter, window, columnNumber)
    lefAlignedLabel(aanbieder, counter, window, columnNumber + 1)
    lefAlignedLabel(startTijd, counter, window, columnNumber + 2)
    lefAlignedLabel(eindTijd, counter, window, columnNumber + 3)

    button = Button(master=window, text='Aanmelden', command= lambda: popupSignUp(idx, window, movieTitle))
    button.grid(row=(counter), column=columnNumber+3)
...
def client():
    ...
    beheerderbutton.grid(row=0, column=9)
    ...
    for idx in range(len(moviesVandaag)):
        ...
        movieLabel(moviesVandaag[idx], vandaagcounter, idx, root, 1)

    for jdx in range(len(moviesMorgen)):
        morgencounter = morgencounter + 1
        movieLabel(moviesMorgen[jdx], morgencounter, jdx, root, 6)
